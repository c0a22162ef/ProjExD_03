...
class Beam2:
    ...
    def __init__(self, bird2: Bird2):
        ...
        self._img2 = pg.transform.rotozoom(pg.image.load(f"fig/beam.png"), 0, 2.0)  # 画像surface
        self._rct = self._img.get_rect()  # 画像surfaceに対応したrect
        self._rct.left = bird2._rct.right  # こうかとん2の左側にビームの左側を合わせる
        self._rct.centery = bird2._rct.centery
        self._vx, self._vy = +1, 0
...
def main():
    ...
    while True:
        ...
        for bomb in bombs:
            bomb.update(screen)
            if bird2._rct.colliderect(bomb._rct):
                # ゲームオーバー時に，こうかとん2画像を切り替え，1秒間表示させる
                bird.change_img(2, screen)
                bird2.change_img(8, screen)
                pg.display.update()
                time.sleep(1)
                return
        ...
        if beam2 is not None:  # ビーム2が存在しているとき
            beam.update(screen)
            for i, bomb in enumerate(bombs):
                if beam2._rct.colliderect(bomb._rct):
                    beam2 = None
                    del bombs[i]
